...
//verificar se tem conta vencidas
	...
	if($linhas > 0){
		for($i=0; $i<$linhas; $i++){
				...
				$id_conta = $res[$i]['id'];	
				$pdo->query("UPDATE receber set hora_alerta = '$hora_random_script', data_alerta = null, cobrar_sempre = 'Sim' where id = '$id_conta'");					
		        
			}
	}


echo 'Total de '.$linhas.' cobranças agendadas para serem disparadas na próxima hora!!';
